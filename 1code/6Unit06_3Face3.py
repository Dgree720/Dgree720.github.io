...
mouth_normal = cv2.imread("pic/m6.png")  # pic of mouth
...
while cap.isOpened():
    ...
    if results.multi_face_landmarks:
        for f_landmarks in results.multi_face_landmarks:
            ...
            mouth_len = int(
                (f_landmarks.landmark[17].y * h) - int(f_landmarks.landmark[0].y * h)
            )
            mouth = cv2.resize(
                mouth_normal, (mouth_len * 3, mouth_len)
            )  # resize the pic (m6.png) into the right size
            mouth_gray = cv2.cvtColor(mouth, cv2.COLOR_BGR2GRAY)  # convert pic to gray
            _, mouth_mask = cv2.threshold(
                mouth_gray, 25, 255, cv2.THRESH_BINARY_INV
            )  # make a mask
            img_height, img_width, _ = mouth.shape  # find the img_height and img_width
            # the center is between point #13 and #14
            x, y = (
                int(f_landmarks.landmark[13].x * w - img_width / 2),
                int(
                    ((f_landmarks.landmark[13].y + f_landmarks.landmark[14].y) / 2) * h
                    - img_height / 2
                ),
            )
