...
selfie_segmentation = mp.solutions.selfie_segmentation.SelfieSegmentation(
    0
)  # 0 general-purpose, 1 landscape images
...
while cap.isOpened():
    ...
    results = selfie_segmentation.process(
        imgrgb
    )  # process selfie_segmentation and get result
    ...
    cv2.imshow("Unit04_1 | 322022 |original", img)
    cv2.imshow("Unit04_1 | 322022 |selfie_segmentation1", output_image)
