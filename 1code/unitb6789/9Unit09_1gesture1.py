...
def vector_2d_angle(
    v1, v2
):  # Calculate the angle based on the coordinates of two points
    v1_x = v1[0]
    v1_y = v1[1]
    v2_x = v2[0]
    v2_y = v2[1]
...
def hand_angle(
    hand_,
):  # Calculate the angle of the finger based on the 21 node coordinates
    angle_list = []
...
def hand_pos(finger_angle):  # return the corresponding gesture name.
    ...
    if (
        f1 < 50 and f2 >= 50 and f3 >= 50 and f4 >= 50 and f5 >= 50
    ):  # <50 straight finger,>=50 curled finger
        return "good", False
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for h_landmarks in results.multi_hand_landmarks:
            ...
            for i in (
                h_landmarks.landmark
            ):  # Convert to coordinates and record them in the 'finger_points'
                x = i.x * w
                y = i.y * h
            ...
            if finger_points:
                finger_angle = hand_angle(
                    finger_points
                )  # Calculate finger angles and return a list of length 5
                text, blur = hand_pos(
                    finger_angle
                )  # Retrieve the content returned by the gesture
                cv2.putText(img, text, (30, 120), 1, 5, (0, 0, 255), 10, 1)

                # implement blurring and flipping when middle-finger
                if blur:
                    img = cv2.GaussianBlur(img, (55, 55), 0)
                    img = cv2.flip(img, 0)

    cv2.imshow("Unit09_1 | StudentID | gesture1", img)
